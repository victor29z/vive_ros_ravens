...
XMATE3_URDF_PATH = 'xmate3/xmate3_with_gripper.urdf'
...
class Environment(gym.Env):
  ...
  def __init__(self,
    ...
    self.agent_cams = cameras.D415_infront.CONFIG
    ...
    if disp:
      ...
      p.resetDebugVisualizerCamera(
          cameraDistance=1.1,
          cameraYaw=90,
          cameraPitch=-45,  #-25 in original setup
  ...
  def step_simple(self, action = None):
      ...
      if action != None:
          ...
          if grasp == 1:
            ...
            # add laser marker on desktop
          marker_head_point = [ee_pose[0][0], ee_pose[0][1], 0.05]
          marker_tail_point = [ee_pose[0][0], ee_pose[0][1], 0.06]
          ...
          # marker_traj_head = [ee_pose[0][0], ee_pose[0][1], ee_pose[0][2]]
          # marker_traj_tail = [ee_pose[0][0], ee_pose[0][1], ee_pose[0][2]+0.01]
          #
          # p.addUserDebugLine(marker_traj_head,
          #                    marker_traj_tail,
          #                    lineColorRGB=[0, 1, 0],
          #                    lifeTime=0,
          #                    lineWidth=1)
  ...
  def movej(self, targj, speed=0.5, timeout=5):
